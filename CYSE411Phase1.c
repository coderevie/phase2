...
int readSock(int sock, char *buf, size_t len)
{
	...
	return bytesread;//returns number of bytes read from socket	
}
//writeArticle opens a file then writes a socket to the file (if a file exists)then closes the file and releases buffer
void writeArticle(int sock, FILE *logfile, char *action)
{
	...
	strncat(path, &action[1], sizeof(path));//copy the location of the article on the disk

	logData(logfile, "user writing article: %s", path);//writes to log file

	file = fopen(&action[1], "w");//opens the file in writing mode

	if (!file)//if not file is there dont write socket
	{
		writeSock(sock, FILENOTAVAIL, sizeof(FILENOTAVAIL));//writes a socket that will not be available
		return;
	}
	//writes socket to the file
	writeSock(sock, BEGINFILE, sizeof(BEGINFILE));

	while (1)//always 1(true) so it will always go through the loop
	{
		...
		for (y = 0; y < x; ++y)
		{
			if (buf[y] == '!')
			{
				if (buf[y+1] == '!' && buf[y+2] == '!')//checks if end of buffer, in array [_,_,_]
				{
					buf[y] = 0x0;
					complete = 1;
				}
		...
		if (complete)//end of loop, if complete==1 is the same as true
		{
			break;//break to exit while loop
		}
	}

	writeSock(sock, ARTICLEWROTE, sizeof(ARTICLEWROTE));//write the sock to articlewrote
	fclose(file);//closes the file
}

//opens the file to reads it then closes it
void readArticle(int sock, FILE *logfile, char *action)
{
	...
	if (!file)//checks if there is a file present, if not go into the if statement
	{
...
//runs a system command and logging it into the log file
void command(FILE *log, int sock, char *action)
{
	logData(log, "executing command: %s", &action[1]);
	system(&action[1]);
}
//removed a duplicate void command
...
void addUser(FILE *log, int sock, char *action)
{
	...
	*p = 0x0;//pointer p pointing to 0x0
...
void adminFunctions(FILE *logfile, int sock)
{
	...
	while (1)
	{
		...
		if (action[0] == ADD_USER)//checks if you need to add a user
		{
			addUser(logfile, sock, action);	//user gets added and has access to logfile, socket, and action
		}
		else if (action[0] == COMMAND) 
		{
			command(logfile, sock, action);	//Admin command execution 
		}
		else
		{
			logData(logfile, "unknown action: %x", action[0]);//logging a unknown action
		}
	}
...
int userFunctions(FILE *logfile, int sock, char *user)
{
	...
	if (0 == strncmp(user, "admin", 5))// If user named admin, give user admin functions
	{
		adminFunctions(logfile, sock);
		return 0;
	}
	
	while (1)//always 1(true) will enter loop 
	{
		writeSock(sock, READY, sizeof(READY));//writing a socket
		memset(action, 0, sizeof(action));
		len = readSock(sock, action, sizeof(action));	
		
		//different actions a user can do
...
int authenticate(FILE *logfile, char *user, char *pass)
{
	...
	snprintf(search, sizeof(userfile)-1, "stat %s`ls %s | grep %s`", USERPATH, userfile);
	ret = system(search);
	...
	if (!file)//if not file opens the log file to check password
	{
...
void handleConnection(FILE *logfile, int sock)
{
	...
	if ((unsigned int)segmentcount * 8 > sizeof(argbuf))
	{
	...
	logData(logfile, buffer);//writes buffer into logfile
	...
	if (ret != 1)
	{
		...
		logData(logfile, buffer);//logs the reason the authentication failed
		return;
	}

	logData(logfile, "user %s authenticated!", user);//writes to logfile that user has been authenticated
...
void mainLoop(FILE *logf, int sock)
{
	...
	struct socklen_t *clientlen = 0;
	struct pid_t *offspring = 0;
...
int main(int argc, char *argv[])
{
	...
	if (!logf)//checks if log file is created and can open it returning a message
	{
	...
	logData(logf, "intial socket setup complete");//logs that the socket was set up properly
